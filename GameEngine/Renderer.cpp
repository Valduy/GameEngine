...
ID3D11Device* engine::Renderer::GetDevice() {
	return device_.Get();
}

ID3D11DeviceContext* engine::Renderer::GetContext() {
	return context_.Get();
}

engine::Renderer::Renderer(HINSTANCE instance, LPCWSTR window_name, unsigned width, unsigned height)
...
void engine::Renderer::BeginRender() {
	context_->ClearState();
...
void engine::Renderer::EndRender() {
	context_->OMSetRenderTargets(0, nullptr, nullptr);
	swap_chain_->Present(1, /*DXGI_PRESENT_DO_NOT_WAIT*/ 0);
}

HRESULT engine::Renderer::CreateDeviceAndSwapChain() {
...
HRESULT engine::Renderer::GetBackTexture() {
...
HRESULT engine::Renderer::CreateRenderTargetView() {
	return device_->CreateRenderTargetView(back_texture_.Get(), nullptr, &render_target_view_);
}

HRESULT engine::Renderer::CreateRasterState() {
